...
export function QuoteDocuments({ quoteId, documents, onRefresh }: QuoteDocumentsProps) {
  ...
  const handleDelete = async (docId: string) => {
    ...
    try {
      const response = await fetch(`/api/quotes/${quoteId}/documents/${docId}`, {
        method: 'DELETE',
      });
  ...
  const handleDownload = (docId: string, fileName: string) => {
    ...
    link.href = `/api/quotes/${quoteId}/documents/${docId}`;
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-primary-500">
                      {formatFileSize(doc.fileSize)} • {doc.uploadedBy.fullName} • {formatDate(doc.createdAt)}
                    </p>
                  </div>
                </div>
