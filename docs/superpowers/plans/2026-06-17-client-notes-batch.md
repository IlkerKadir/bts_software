...
| B9 | Teklif Takip panel + admin export | New feature | High | Yes (1 table + enums) | ✅ |
...
**Done (12/12 — COMPLETE):** A1, A2, A3, B1(newlines), B2, B3, B4, B5, B6, B7, B8(column + admin export), B9. All on the branch, 620 tests passing. Optional leftover: B1 part 2 (bold/bullets rich text) only.
**Migrations applied locally + recorded (3 total):** add_project_location, add_company_types, add_quote_tracking — all additive, safe for `prisma migrate deploy`.
**Reviews DONE** (B4, B2, B9 all reviewed — no critical issues). Findings addressed in commit `022c1e5`: B9 visibility enforcement on tracking/interactions routes, lost-fields-only-when-KAYBEDILDI, export search incl. project.name + UTC dates, ZodError trimmed; B4 insertBeforeId reset + explicit onAddHeader(); B2 out-of-range page snap-back (4 lists).
**Accepted (not fixed — low impact):** B4 stale-snapshot clobber on insert (low-probability; synchronous snapshot kept to avoid setState-timing pitfall); B2 hydration flash on hard-reload (dev-only warning; hook refactor risked a worse clobber). Both noted should they ever surface.
...
- Static (overwrite) fields: Önem Sırası (A/B/C/D), Başarı % (number), Beklenen Sipariş Tarihi (date).
