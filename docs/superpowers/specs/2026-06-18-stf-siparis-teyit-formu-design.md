...
3. **Access control hardening (STILL OPEN — flagged by Phase 2 final review as the ship-blocker):** orders GET/PATCH/**PUT** are session-only. PUT (full STF edit: wholesale item replace + header/footer + totals) has NO permission check and NO status gate — *any* logged-in user can rewrite *any* STF in *any* state (incl. GONDERILDI/TAMAMLANDI already sent to a customer). DELETE by contrast requires `canDelete` + HAZIRLANIYOR. Add a shared `canAccessOrder(order, user)` mirroring `quotes/[id]`'s `canAccessQuote` (creator / project visibility + managers), apply to GET/PATCH/PUT, and gate PUT to non-terminal statuses. **Decide the edit-permission model with the owner before shipping Phase 2 to customers.**
